...
This task is probably a non-starter if you have more than a few hundered lines of code. This is where [untangle](untangle) can save you time. [untangle](untangle) will separate the long script into single function files in a subdirectory and keep the body script intact.
...
This is where [pretty_namespace](#prettyns) comes in. This function will go through your script and attach namespaces for you, with the same logic as the search path.
...
The premise of [makeOxygen](#makeoxygen) is to expand on the default skeleton in `RStudio`, so basic fields are in the output by default. Each field is given with a relevant placeholder giving a hint what is expected. The following is the meat add to these bones:
...
It is also important to update the package description file Imports field. This can be done for you with [makeImport](#makeimport), by either creating a new Imports field or updating an existing one.
