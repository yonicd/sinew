...
```{r}
makeImport(script = pkg_dir_r,format = 'description')
```
...
```{r}
#single file
makeImport(script = file.path(pkg_dir_r,'yy.R') ,format = 'oxygen')
```

```{r}
#whole directory
makeImport(script = pkg_dir_r,format = 'oxygen')
```
...
```{r}
#with cut
makeImport(script=file.path(pkg_dir_r,'yy.R'),format = 'oxygen', cut = 1)
```
