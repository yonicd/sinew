...
`sinew` can help turn the around that headache into a `CRAN` ready package in a few short steps
  ...
  6. Run `sinew::makeImport` to populate the `Imports` field of the `DESCRIPTION` file.
...
It is also important to update the package `DESCRIPTION` file `Imports` field. This can be done for you with `sinew::makeImport`, by either creating a new `Imports` field or updating an existing one.

```{r}
sinew::makeImport(pkg_dir_R,format = 'description',desc_loc = pkg_dir)
```
